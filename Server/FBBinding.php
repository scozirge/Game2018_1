...
$result = mysql_query("SELECT * FROM ".$db_name.".playeraccount WHERE `FBID`='".$FBID."'",$con_l);
$dataNum = mysql_num_rows($result );

if ($dataNum == 0)
{
	$checkAC = mysql_query("SELECT * FROM ".$db_name.".playeraccount WHERE `account`='".$ac."'",$con_l);
	$acNum = mysql_num_rows($checkAC );
	if ($acNum == 0)//server找不到FBID 也找不到相同帳號 創立新帳號在server並將資料跟FBID傳到server上
	{
	...
	else//server找不到FBID 但找到相同帳號 將FBID跟現在資料存到servery上
	{
...
else
{
	...
	{
		//帳號檢查
		if($ac==$row['account'])//server找到FBID 且跟現在本基帳號依樣 將現在資料跟FBID存到server
		{
			//取資料
			$AC=$ac;
			$name=$row['name'];
		...
		else//server找到FBID 但跟本基帳號不一樣 依造FB帳號的server資料傳到本機
		{
			//取資料
			$serverAC=$row['account'];
		   //對帳號進行加密
			$rep = new Crypt3Des (); // new一個加密類
			$ACPass=$rep->encrypt ( "u.6vu4".$serverAC."gk4ru4");
			$name=$row['name'];
			//使用server資料
			$Score=$row['score'];
			$kills=$row['kills'];
			...
			mysql_select_db($db_name , $con_w) or die ("Fail:3:" . mysql_error());
			//使用server資料
			$set = mysql_query("UPDATE `playeraccount` SET `SignInTime` = '".$LastSignIn."' WHERE `Account` = '".$serverAC."' ",$con_w);
			//更新資料的回傳結果
			if($set)
			{
				//計算執行時間
				...
				$executeTime = $time_end - $time_start;
				//使用server資料
				die("ChangeAC:".$serverAC.",".$ACPass.",".$name.",". $Score.",".$kills.",".$shot.",".$criticalHit.",".$death.",".$criticalCombo. ": \nExecuteTime=".$executeTime);
			}
			else
			{
				die("Fail:12");
